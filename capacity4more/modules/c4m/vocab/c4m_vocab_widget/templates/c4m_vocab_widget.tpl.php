<div class="form-group btn-group <?php print($vocabulary_machine_name); ?>">
  <div class="checkboxes-wrapper">
    <div>
      <button type="button" ng-click="togglePopover('<?php print($vocabulary_machine_name); ?>', $event)" class="btn btn-primary fa fa-plus">&nbsp;<?php print(t('Select @name', array('@name' => $vocabulary_label))); ?></button>
    </div>
    <div class="selected-values" ng-show="data.<?php print($vocabulary_machine_name); ?>">
      <div class="value row" ng-show="value === true && data.<?php print($vocabulary_machine_name); ?>[key]" ng-repeat="(key, value) in model">
        ...
        <div class="child col-sm-6">
          <span ng-show="model[child.id] === true" ng-repeat="(childkey, child) in data.<?php print($vocabulary_machine_name); ?>[key].children">
    ...
    <div class="popover right hidden-checkboxes" ng-hide="popups.<?php print($vocabulary_machine_name); ?> != 1">
      ...
      <div class="popover-content">
        ...
        <div class="checkbox" ng-repeat="item in filteredTerms.<?php print($vocabulary_machine_name); ?>">
          <label>
            <input type="checkbox" ng-name="type" title="{{item.label}}" ng-model="model[item.id]" ng-change="updateSelectedTerms(item.id, '<?php print($vocabulary_machine_name); ?>')"> {{item.label}}
          </label>
          <ul ng-show="item.children && model[item.id] == true" class="indent">
            <li ng-repeat="child in item.children">
              <label>
                <input type="checkbox" ng-name="type" ng-model="model[child.id]" ng-change="updateSelectedTerms(child.id, '<?php print($vocabulary_machine_name); ?>')"> {{child.label}}
              </label>
            </li>
          </ul>
        </div>
