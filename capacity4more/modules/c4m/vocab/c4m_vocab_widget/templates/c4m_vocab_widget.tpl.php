    ...
    <div class="popover right hidden-checkboxes" ng-hide="popups.<?php print $vocabulary_machine_name; ?> != 1">
      ...
      <div class="popover-content">
        ...
        <div class="checkbox" ng-repeat="item in filteredTerms.<?php print $vocabulary_machine_name; ?>">
          <label>
            <input type="checkbox" data-target="{{item.id}}" ng-name="type" title="{{item.label}}" ng-model="model[item.id]" ng-change="updateSelectedTerms(item.id, '<?php print $vocabulary_machine_name; ?>')"> {{item.label}}
          </label>
          <ul ng-show="item.children && model[item.id] == true" class="indent">
            <li ng-repeat="child in item.children">
              <label>
                <input type="checkbox" data-target="{{item.id}}" ng-name="type" ng-model="model[child.id]" ng-change="updateSelectedTerms(child.id, '<?php print $vocabulary_machine_name; ?>')"> {{child.label}}
              </label>
            </li>
          </ul>
        </div>
